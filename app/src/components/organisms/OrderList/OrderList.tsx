...
export default function OrderList(props: IProps) {
  function getHeader() {
    if (props.orderLines && props.orderLines.length) {
      return (
        <S.Header>
          {Object.keys(props.orderLines[0]).map(
            (value: string, index: number) => {
              return (
                <S.HeaderValue
                  key={index}
                  widthPercentage={
                    100 / Object.keys(props.orderLines![0]).length
                  }
                >
          ...
          {Object.keys(groupedOrderLines).map((key: any) => {
            return (
              <React.Fragment key={key}>
                ...
                {groupedOrderLines[key].map(
                  (mintOrderLine: MintOrderLineType, index: number) => {
                    return (
                      <S.DetailLine
                        key={index}
                        ...
                        ownerLine={
                          props.owner
                            ? props.owner === mintOrderLine.owner
                            : false
                        }
                      >
